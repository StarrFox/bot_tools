...
from bot_stuff.Logging_handler import DiscordHandler

class logger(commands.Cog):
    """
    error handler and bot logging
    needs bot_tools bot and channel to be passed in load_extension
    """
    ...
    async def on_guild_join(self, guild):
        self.logs.info(f"Joined {guild.name} owned by {guild.owner}({guild.owner.id}) "
                       f"with {guild.member_count} members.")

    @commands.Cog.listener()
    async def on_guild_remove(self, guild):
        self.logs.info(f"Left {guild.name}({guild.id}) owned by {guild.owner}({guild.owner.id}) "
                       f"with {guild.member_count} members.")

    @commands.Cog.listener("on_message")
    async def pm_logger(self, message):
        if message.guild is None and not message.author == self.bot.user:
            self.logs.info(f"Got PM from {message.author.name}({message.author.id}) "
                           f"with content \"{message.content}\" and id {message.id}")

    @commands.Cog.listener()
    async def on_command_completion(self, ctx):
        self.logs.info(
            f"Ran command {ctx.command.name} in {ctx.guild.name}({ctx.guild.id}) by {ctx.author}({ctx.author.id}) "
            f"with message id {ctx.message.id}"
        )
    ...
    async def on_command_error(self, ctx, error):
        ...
        self.logs.error(
            f"Error executing command {ctx.command.name}",
